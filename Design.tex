...
Для начала надо скачать \verb|.sty| файл в директорию вашего документа для использование наших предопределенных команд и стилей 
...
\begin{code}
	...
	\end{figure}
\end{code}
Этот кусок кода рисует отрезок соединяющий точки с координатами $(0,0)$ и $(2~\text{сm},0)$.  Вообще, стандартной единицой длины в tikz являются сантиметры, но можно, например, в качестве координаты точки использовать $(2\text{pt}, 0)$.
...
\begin{code}
	...
	\end{tikzpicture}
\end{code}
Также здесь продемонстрирован пример подписи объекта с использованием команды \verb|node|. Запись \verb|++(2,0)| означает сдвиг отнсительно предыдущей координаты на $2~\text{см}$.
...
Код, компилируемый в этот рисунок, представлен ниже. Матрицы преобразований:
\[ M_2 = 
\begin{pmatrix}
	1&0\\0.5&0.5
\end{pmatrix}
...
\subsection{Повороты координат и запоминание точки}

Если значительная часть картинки должна быть сдвинута и повернута, то можно использовать окружение \verb|scope|, и использовать аргумент \verb|[shift={(x,y)},rotate=z]|, где \verb|x,y| -- положение центра координат \verb|scope| на холста, а \verb|z| -- угол в градусах, на который будет повернуты координаты внутри \verb|scope| относительно координат \verb|scope|. При этом бывает полезно запомнить точку, координаты которой внутри scope задаются простым способом с помощью команды \verb|coordinate (name)|. Далее \verb|(name)| можно использовать наравне с любыми другими точками.
...
В виде \verb|(20:1)| задается точка, удаленная на расстояние $1~\text{cm}$ от начала отсчета, такая, что ее радиус-вектор составляет угол $20^\circ$ с направлением оси $x$ по часовой стрелке. Также корректно использовать \verb| ++ (20:1)| -- сдвиг от текущего положения на $1~\text{cm}$ в направлении составлющем $20^\circ$ о осью $x$.
...
\begin{enumerate}
	...
	\item Для отрисовки воды используется аргумент \verb|water|, которые автоматически заливает фигуру голубым полупрозрачным цветом.
...
В стандартных случаях, как например подпись длины прямоугольника, можно использовать команды \verb|\dist| и \verb|\smalldist|:
\begin{figure}[H]
\begin{minipage}{0.4\linewidth}
		\centering	
	\begin{tikzpicture}[example, >=latex']
		...
		\smalldist{(0,0)}{(0,0.5)}{180}{left}{0.5 cm}
	\end{tikzpicture}
\end{minipage}
\begin{minipage}{0.6\linewidth}
\begin{code}
...
\begin{minipage}{0.4\linewidth}
	...
	\begin{tikzpicture}[example, >=latex']
		...
		\smalldist[1.5][0.3]{(0,0)}{(0,0.5)}{180}{left}{0.5 cm}
	\end{tikzpicture}
\end{minipage}
\begin{minipage}{0.6\linewidth}
	\begin{code}
...
Команды \verb|\dist| и \verb|\smalldist| имеют 5 обязательных (указываются в фигурных скобках \verb|{}|) и 2 опциональных аргумента (указываются в квадратных скобках \verb|[]| \textbf{перед обязательными}). 
\begin{itemize}
...
\begin{tikzpicture}[example, >=latex']
	...
	\draw[rotate=30,double](0.75,0) arc (0:50:0.75);
...
Отрисовка этих объектов абсолютно аналогична недвижимости.
\begin{figure}[H]
\begin{minipage}{0.1\linewidth}
	\centering	
	\begin{tikzpicture}[example, >=latex']
		\draw[thick] (0,2) arc (23.6:-23.6:5);
...
Для отрисовки электрических схем используется пакет \verb|circuittikz|. Рекомендуется использовать аргументы \verb|european resistors| и \verb|american inductors| у всего рисунка
\begin{code}
%	in preamble:
...
Резистору соотвествует аргумент \verb|R|. Потенциометру соотвествует аргумент \verb|pR|, длина его среднего выхода равна 0.56 см.
\begin{figure}[H]
\begin{minipage}{0.2\linewidth}
	\centering	
	\begin{tikzpicture}[example]
...
Для работы с третьим выходом потенциометра даем уникальное название name потенциометру, а далее обращаемся к этой координате, как к name.wiper (\textit{в примере выше это} \verb|pR.wiper|).
...
\section{Лайфхаки}
\subsection{Вертикальные подписи для электроники}
\begin{enumerate}
